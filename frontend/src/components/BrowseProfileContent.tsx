...
export default function BrowseProfileContent() {
  ...
  useEffect(() => {
    const fetchProfiles = async () => {
      ...
      try {
        ...
        const url = value
          ? `${baseUrl}/api/browse-profile?filter=${filter}&value=${value}`
          : `${baseUrl}/api/browse-profile`;
      ...
      <div className="flex flex-col gap-6">
        {profiles.map((user) => {
          ...
          return (
            <div
              key={user.id}
              className="bg-white rounded-xl shadow p-5 flex flex-col sm:flex-row items-center gap-5 hover:shadow-lg transition"
            >
              ...
              <div className="flex-1 text-center sm:text-left">
                ...
                {user.skills && (
                  <p className="mt-3 text-gray-700">
                    <strong>Interests:</strong> {user.skills}
                  </p>
                )}
              </div>
